...
def load_datasets(config, phases, logfile=None):
    ...
    datasets = {x: ReviewsDataset(data=join(config['dir'], datafiles[x]), review_vocab=review_vocab, summary_vocab=summary_vocab)
                for x in phases}
    if 'fixed_weights' not in config:
        samplers = {x: datasets[x].get_sampler() if x == 'train' else None for x in phases}
    else:
        samplers = {x: datasets[x].get_sampler(np.array([4.0, 4.0, 1.0])) if x == 'train' else None for x in phases}
...
def main(config):
    ...
    if config['model']['params']['use_summary']:
    ...
    if 'reload' in config['model']:
        reloadPath = join(config['save_dir'], config['model']['reload'])
    ...
    if config['mode'] == 'train':
        ...
        model = train_model(model, dataloaders, criterion, optimizer, exp_lr_scheduler, save_dir,
                            ...
                            best_fscore=best_fscore, start_epoch=startEpoch, logfile=logfile)
